...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info("Запуск приложения (environment=%s, llm_url=%s, model=%s)", settings.environment, settings.llm_base_url, settings.target_model_name)

    # Stateless Глоссарий (без локальных файлов/БД)
    glossary = GlossaryService()

    # LLM-клиент REST (LM Studio / LiteLLM / vLLM)
    ...
    logger.info("Stateless приложение готово к обработке запросов.")
    yield
